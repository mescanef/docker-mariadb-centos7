...
SetupRootUser()
{
	...
	mysql -uroot -h127.0.0.1 -e "CREATE USER '$MARIADB_USER'@'%' IDENTIFIED BY '$MARIADB_PASS';"
	mysql -uroot -h127.0.0.1 -e "GRANT ALL PRIVILEGES ON *.* TO '$MARIADB_USER'@'%' WITH GRANT OPTION;"    
	echo "=> Done!"
	echo "========================================================================"
    ...
    mysql -uroot -h127.0.0.1 -e "DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';"
    mysql -uroot -h127.0.0.1 -e "DROP DATABASE test;"
    mysql -uroot -h127.0.0.1 -e "DELETE FROM mysql.user WHERE password='';"     
}
...
if [[ ! -d $VOLUME_HOME/mysql ]]; then	
    ...
    echo "=> Done!"  
	# Set MySQL REPLICATION - MASTER IF required...
	if [ -n "${REPLICATION_MASTER}" ]; then 
    	...
    	if [ ! -f /repliation_configured ]; then
        	...
        	mysql -uroot -h127.0.0.1 -e "CREATE USER '${REPLICATION_USER}'@'%' IDENTIFIED BY '${REPLICATION_PASS}'"
        	mysql -uroot -h127.0.0.1 -e "GRANT REPLICATION SLAVE ON *.* TO '${REPLICATION_USER}'@'%'"
        	echo "=> Done!"
            SetupRootUser
	...
	# Set MySQL REPLICATION - SLAVE IF required...
    	...
    	if [ -n "${MYSQL_PORT_3306_TCP_ADDR}" ] && [ -n "${MYSQL_PORT_3306_TCP_PORT}" ]; then
        	if [ ! -f /repliation_configured ]; then
            	...
            	mysql -uroot -h127.0.0.1 -e "CHANGE MASTER TO MASTER_HOST='${MYSQL_PORT_3306_TCP_ADDR}',MASTER_USER='${MYSQL_ENV_REPLICATION_USER}',MASTER_PASSWORD='${MYSQL_ENV_REPLICATION_PASS}',MASTER_PORT=${MYSQL_PORT_3306_TCP_PORT}, MASTER_CONNECT_RETRY=30"
            	echo "=> Done!"
            	SetupRootUser
            	touch /repliation_configured
       		else
